...
X = df_cleaned[['Meal', 'Day of the Week', 'Adjusted Time', 'Location', 'Month', 'Day of the Month']]
y = df_cleaned['# Guests']
...
if st.button("Generate Calendar"):
    ...
    for day in dates:
        ...
        input_data = pd.DataFrame({
            'Meal': [label_encoders['Meal'].transform([meal])[0]],  # Assuming you have 'meal' value
            'Day of the Week': [label_encoders['Day of the Week'].transform([day_name])[0]],
            'Adjusted Time': [label_encoders['Adjusted Time'].transform([time_of_day_input])[0]],  
            'Location': [label_encoders['Location'].transform([location_encoded])[0]],
            'Month': [day.month],
        ...
        # Make prediction
        prediction = model.predict(input_data_scaled)
        heatmap_data.append({"Date": day, "Day": day.day, "Day of Week": day.weekday(), "Guests": int(prediction[0])})
